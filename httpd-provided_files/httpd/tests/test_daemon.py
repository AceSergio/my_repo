...
# Arguments communs pour lancer le binaire
# On simule les arguments que config_reader.sh générerait
BASE_CMD = [
    BIN,
...
def check_pid_running(pid):
    ...
    try:
        # Le signal 0 ne tue pas le processus mais vérifie son existence
        os.kill(pid, 0)
    except OSError:
        return False
    return True
...
def run_daemon_test():
    ...
    time.sleep(0.5) # Laisser le temps au fork
    ...
    time.sleep(1.5) # Restart a un sleep(1) dans le code C + délai fork
    ...
    # Vérifier que l'ancien est bien mort
